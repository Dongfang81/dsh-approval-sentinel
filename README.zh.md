...
**DeepSeek Harness 的「帮我批准」：当一条权限审批长时间无人回应时，由一个新拉起的风险评估 agent 审查请求——低风险操作自动放行、危险操作自动拒绝、拿不准时继续等你确认。**
...
3. `allow` → 放行；`reject` → 拒绝；`wait` → 继续等人工确认（最多 `maxWaits` 轮后仍无人确认则失败关闭为拒绝）；
...
| 拿不准 | `ESCALATE` → 立即转人工。 | `wait` → 重新武装人工宽限期（不重复评审），`maxWaits` 次后失败关闭为拒绝。 |
...
approval/request（waterfall 钩子）
   ...
   ├─ denyPatterns 命中 ────────────────────────► 'rejected'   （确定性，无模型调用）
   ├─ allowPatterns 命中 ───────────────────────► 'allowed-once'
   ├─ 启动人工通道（next()）并与宽限计时器赛跑
   │    ├─ 人工在宽限内回应 ──────────────────────► 以人工决定为准
   │    ├─ 无回答者（'unavailable'）──────────────► headless 审查（可加 headlessGraceMs）
   │    └─ 宽限超时 ─────────────────────────────► 全新风险评估 subagent
   │         ├─ allow ──────────────────────────► 'allowed-once'
   │         ├─ reject ─────────────────────────► 'rejected'
   │         └─ wait ───────────────────────────► 重新武装人工宽限；maxWaits 次后 → 'rejected'
   │
   └─ 每次自动决定都会被审计（approval/asked + approval/decided）
      并向会话注入一条消息
```

评审 subagent 通过 `ctx.subagents` 的 `spawn` 系提供者（全新 agent，`inheritsParentContext: false`）拉起，限制为**无工具**（`toolFilter: { allow: [] }`），必须通过结构化输出工具返回结论。其提示词把请求视为不可信数据，并指示它对破坏性、不可逆或工作区外的操作优先选择 `wait`/`reject`。
