...
export async function runApprovalFlow({ req, next, cfg, deps }) {
  ...
  const winner = await raceWithTimeout(human, cfg.graceMs);
  if (req.signal?.aborted === true) return "cancelled";
...
export function buildNotice(req, state, riskLevel, rationale, info) {
  ...
  const detail = truncate(rationale ?? "", 300);
  if (info.source === "allow-rules") {
    return `[帮我批准] 「始终允许」规则命中，请求「${req.toolName}: ${reason}」→ ${verb}（风险: ${riskLevel ?? "unknown"}）。理由: ${detail}`;
  }
  const when = info.source === "quick"
    ...
    : info.headless
      ? "无人工审批通道（headless）"
      : "人工宽限期超时";
  const by = info.source === "quick" ? "确定性规则" : "风险评估 agent";
  return `[帮我批准] ${when}，${by} 审查了请求「${req.toolName}: ${reason}」→ ${verb}（风险: ${riskLevel ?? "unknown"}）。理由: ${detail}`;
}
...
async function raceWithTimeout(promise, ms) {
  ...
  const timeout = new Promise((resolve) => {
    // A local timer (not the injected sleep) so the race can ALWAYS clear it:
    // an unreferenced grace timer would keep the process alive after the flow
    // settled, stalling test runners and CI.
...
/**
 * Throttled system-notification gate: decides whether a notification of
 * `kind` for `sessionId` may be sent right now.
 * @param {object} input
 * @param {boolean} input.enabled - the per-kind config switch.
 * @param {number} input.minIntervalMs - minimum gap between two notifications of the same kind+session (ms).
 * @param {number|undefined} input.lastTs - previous send timestamp for this kind+session.
 * @param {number} [input.now] - clock (default Date.now()).
 * @returns {boolean} true when the notification should be sent.
 */
export function shouldNotify({ enabled, minIntervalMs, lastTs, now = Date.now() }) {
  if (!enabled) return false;
  if (now - (lastTs ?? 0) < Math.max(0, minIntervalMs)) return false;
  return true;
}

/** Best-effort JSON parse (tool-call arguments arrive as a JSON string). */
...
/**
 * Find an `ask_user_question` tool call inside an assistant message content
 * block list and return a short human-readable question text.
 * @param {readonly any[]|undefined} content - assistant message content blocks.
 * @param {number} [maxChars]
 * @returns {string|undefined} the first question's text (or a fallback), or
 *   undefined when no ask_user_question call is present.
 */
export function extractQuestionText(content, maxChars = 120) {
  if (!Array.isArray(content)) return void 0;
  for (const block of content) {
    if (block === null || typeof block !== "object") continue;
    if (block.type !== "tool-call" || block.name !== "ask_user_question") continue;
    const args = tryParseJson(block.arguments);
    if (args === void 0) return "需要你的输入";
    const first = Array.isArray(args.questions) ? args.questions[0] : void 0;
    const text = (first !== void 0 && typeof first === "object" && typeof first.question === "string")
      ? first.question
      : typeof args.question === "string"
        ? args.question
        : "";
    return truncate(text.length > 0 ? text : "需要你的输入", maxChars);
  }
  return void 0;
}
