...
export function apply(ctx, config) {
  ...
  ctx.inject(["settings"], (sctx) => {
    try {
      ...
      // Never let a settings registration problem take down the plugin tree:
      // degrade to the entry config (dynamic() falls back to `base`).
      ctx.logger?.(name).warn(`settings register failed, using entry config: ${error?.message ?? String(error)}`);
      return void 0;
    }
  });
  ...
  // hot-reload for the host policy. Registered through inject so the routes
  // exist even when this plugin applies before the webServer service.
  ctx.inject(["webServer"], (wctx) => {
    ...
    try {
      ...
      ctx.logger?.(name).warn(`webServer route registration failed: ${error?.message ?? String(error)}`);
    }
  });
