...
export const Config = z.object({
  ...
  /** Rounds of `wait` verdict before failing closed to `rejected`. */
  maxWaits: z.natural().min(1).default(2),
  ...
  /** What to do when the assessor itself fails/times out: `wait` hands back to the human (then fail closed after maxWaits), `reject` denies immediately. */
  onAssessError: z.union(["reject", "wait"]).default("wait"),
...
function buildAssessorPrompt({ req, context, workspaceRoot, mode }) {
  return [
    ...
    '- "reject" — if it risks host damage, irreversible data loss, credential or secret exfiltration, system/OS modification, persistence installation, network transmission of sensitive data, or clearly exceeds what a reasonable user would approve.',
    '- "wait" — you cannot confidently classify; the human should decide.',
    "",
    "## Rules",
...
export function apply(ctx, config) {
   ...
   */
  async function assess({ req, cfg, headless, waitRound }) {
  ...
  log("info", `loaded (enabled=${base.enabled}, graceMs=${base.graceMs}, assessTimeoutMs=${base.assessTimeoutMs}, maxWaits=${base.maxWaits})`);
}
