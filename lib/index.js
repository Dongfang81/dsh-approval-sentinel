...
import { runApprovalFlow, buildSessionContext, createGate, VERDICTS, extractEscalationMode } from "./core.js";
...
export function apply(ctx, config) {
  ...
  // scope: `scope.update()` is how "始终允许" rules are persisted.
  ...
  // Listen for the user replying "始终允许" (or "always allow") right after a
  // request was auto-approved — the codex-style "always allow" interaction.
  ...
  log("info", `loaded (enabled=${base.enabled}, graceMs=${base.graceMs}, assessTimeoutMs=${base.assessTimeoutMs}, maxConcurrentAssessments=${base.maxConcurrentAssessments}, allowRules=${base.allowRules?.length ?? 0})`);
}
